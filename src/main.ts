...
function getTzDisplayName(tz: string, lang: 'en' | 'zh'): string {
    ...
    if (matched && matched.zh) {
        const prefixMatch = tz.split('/')[0];
        return `(${TZ_PREFIX[prefixMatch] || ''}) ${matched.zh}`;
    }
    let parts = tz.split('/');
    if (parts.length === 1) return tz;
    let prefixMatch = parts[0];
    let prefix = TZ_PREFIX[prefixMatch] || prefixMatch;
...
class TimeEngine {
    static getTimeParts(tz: string, now: Date) {
        ...
        try {
            type IntlFormat = { formatToParts(d: Date): Array<{type: string, value: string}> };
            ...
            const parts = (formatter as unknown as IntlFormat).formatToParts(now);
...
class TimezoneSuggestModal extends FuzzySuggestModal<string> {
    ...
    getItems() { 
        type IntlTZ = { supportedValuesOf(k: string): string[] };
        return (Intl as unknown as IntlTZ).supportedValuesOf('timeZone'); 
    } 
...
class CustomCityModal extends Modal {
    ...
    onOpen() {
        ...
        type IntlTZ = { supportedValuesOf(k: string): string[] };
        const allZones = (Intl as unknown as IntlTZ).supportedValuesOf('timeZone');
...
class MeridianSettingTab extends PluginSettingTab {
    ...
    private async saveAndRefresh() { await this.plugin.saveSettings(); this.display(); }
    private async saveAndRefreshPreview(previewBox: HTMLElement) { await this.plugin.saveSettings(); this.buildPreview(previewBox); }
    ...
    display(): void {
        ...
        tabAnalog.onclick = async () => { this.plugin.settings.clockFace = 'analog'; await this.saveAndRefresh(); };
        tabDigital.onclick = async () => { this.plugin.settings.clockFace = 'digital'; await this.saveAndRefresh(); };
        ...
        new Setting(secLang).setName(t.langName).addDropdown(drop => drop.addOption('zh', '简体中文').addOption('en', 'English').setValue(this.plugin.settings.language).onChange(async (v: string) => { this.plugin.settings.language = v as 'zh'|'en'; await this.saveAndRefresh(); }));
        ...
        if (isAnalog) {
            ...
            const addColorSetting = (container: HTMLElement, name: string, dictKey: string, valKey: keyof MeridianSettings, customKey: keyof MeridianSettings) => {
                ...
                new Setting(container).setName(name).addDropdown(drop => {
                    ...
                    drop.setValue(this.plugin.settings[valKey] as string).onChange(async (v: string) => { 
                ...
                if (this.plugin.settings[valKey] === 'custom') {
                    new Setting(container).setName(t.colorCustom).addColorPicker(color => {
                        color.setValue(this.plugin.settings[customKey] as string).onChange(async (v: string) => {
                            (this.plugin.settings as Record<string, string>)[customKey] = v;
                            await this.saveAndRefreshPreview(previewBox);
                        });
                    });
                }
            };
            ...
            new Setting(secGeo).setName(t.outerRing).addDropdown(drop => drop.addOption('ticks', t.optTicks).addOption('ticks-dense', t.optDense).addOption('ticks-uniform', t.optUniform).addOption('dots-lines', t.optDotsLines).addOption('numbers', t.optNumbers).addOption('hidden', t.optHidden).setValue(this.plugin.settings.outerRing).onChange(async (v: string) => { 
                this.plugin.settings.outerRing = v as MeridianSettings['outerRing']; 
                if (v === 'dots-lines') this.plugin.settings.innerRing = 'hidden';
                await this.saveAndRefresh(); 
            }));
            const innerSet = new Setting(secGeo).setName(t.innerRing).addDropdown(drop => drop.addOption('ticks', t.optTicks).addOption('numbers', t.optNumbers).addOption('hidden', t.optHidden).setValue(this.plugin.settings.innerRing).onChange(async (v: string) => { this.plugin.settings.innerRing = v as MeridianSettings['innerRing']; await this.saveAndRefreshPreview(previewBox); }));
            ...
            new Setting(secHands).setName(t.secMode).addDropdown(drop => drop.addOption('sweep', t.optSweep).addOption('tick', t.optTick).addOption('hidden', t.optHidden).setValue(this.plugin.settings.secondHandMode).onChange(async (v: string) => { this.plugin.settings.secondHandMode = v as MeridianSettings['secondHandMode']; await this.saveAndRefreshPreview(previewBox); }));
            new Setting(secHands).setName(t.showDate).addToggle(toggle => toggle.setValue(this.plugin.settings.showDate).onChange(async (v) => { this.plugin.settings.showDate = v; await this.saveAndRefreshPreview(previewBox); }));
            ...
            fontList.forEach(font => {
                ...
                card.onclick = async () => { this.plugin.settings.digitalFont = font.id; await this.saveAndRefresh(); };
            });
        }

        const secTz = this.createDetails(containerEl, t.secTz, 'tz');
        new Setting(secTz).setName(t.worldClock).addToggle(toggle => toggle.setValue(this.plugin.settings.isWorldClock).onChange(async (v) => { this.plugin.settings.isWorldClock = v; await this.saveAndRefresh(); }));
